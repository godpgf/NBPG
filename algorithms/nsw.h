                          ...
                          double base = 2, double max_fraction = .02, bool print = true)
        {
            ...
            auto searchNghAndPrune = [&](indexType index, indexType *starting_points, uint32_t sp_num, BeamSearchMemoryCell<indexType> &bsCell)
            {
                ...
                if (ngh.size() > 0)
                {
                    ...
                    for (size_t i = 0; i < bsCell.visited_size; ++i)
                    {
                        max_depth = std::max(bsCell.visited[i].depth, max_depth);
                    }
                    for (size_t i = 0; i < ngh.size(); i++)
                    {
                        if (!bsCell.has_been_seen(ngh[i]))
                        {
                            bsCell.frontier[bsCell.frontier_size++] = QueryNode<indexType>(ngh[i], Points[ngh[i]].distance(Points[index]), max_depth, true);
                            rp_distance_comps++;
                        }
                    }
                    bsCell.frontier_size = merge_union<QNode>(bsCell.frontier, bsCell.frontier_size, bsCell.visited, bsCell.visited_size, bsCell.copy_visited, bsTable.get_max_visited_size(), less);
            ...
            size_t max_batch_size = std::min(getMaxBatchSize(G, max_fraction), this->max_batch_size);
    ...
    public:
        ...
        static void add_neighbors_without_repeats(const rangeType &ngh, IDist *candidates, size_t &candidate_size)
        {
            ...
            for (auto i = 0; i < candidate_size; ++i)
                a.insert(candidates[i].index);
            for (int i = 0; i < ngh.size(); i++)
                if (a.count(ngh[i]) == 0)
                    candidates[candidate_size++].index = ngh[i];
        }
...
#pragma omp parallel for schedule(dynamic, 64) // 每个线程一次处理64个迭代
        ...
        static void inv_edges(MergeEdges<indexType> &merge_edges, const std::vector<SearchResult> &new_out_)
        {
            ...
            for (size_t i = 0; i < new_out_.size(); ++i)
            {
                auto index = new_out_[i].index;
                auto &bsCell = new_out_[i];
                for (size_t j = 0; j < bsCell.prune_size; ++j)
                {
                    auto edge = bsCell.prune_res[j];
                    merge_edges.insert_edge(edge.index, IDist(index, edge.dist));
                }
            }
        ...
        {
            if (merge_edges.get_num_merge() == 0)
                return;
            std::vector<std::vector<std::pair<indexType, float>>> res(merge_edges.get_num_merge());
            std::vector<indexType> all_index(merge_edges.get_num_merge());
#pragma omp parallel for
            for (size_t i = 0; i < merge_edges.get_num_merge(); ++i)
            {
                auto worker_id = omp_get_thread_num();
                indexType index = merge_edges.get_merge_start_index(i);
                all_index[i] = index;
                assert(index >= 0 && index < G.size());
                auto [candidates, candidate_size] = merge_edges.get_merge_end_indices(i, worker_id);
                auto pre_candidate_size = candidate_size;

                auto edges = G[index];
                add_neighbors_without_repeats(edges, candidates, candidate_size);
        ...
        void updateNeighborsForGraph(PR &Points, indexType index, IDist *candidates, size_t pre_candidate_size, size_t &candidate_size, QueryStatic *BuildStats)
        {
            ...
            if (candidate_size <= max_degree)
            {
                ...
                rp_distance_comps = eval_dist_of_new_neighbors(Points, index, candidates, pre_candidate_size, candidate_size, top_degree);
