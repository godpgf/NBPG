...
namespace ant
{
    ...
    struct Linear
    {
        Linear(Optimizer<T>* optimizer, uint32_t in_features, uint32_t out_features, bool use_bias = true)
            : use_bias(use_bias),
              mat(out_features, in_features),
...
#pragma omp parallel for
            for (auto i = 0; i < y_gard.size(); ++i)
            {
                for (auto j = 0; j < y_gard.dimension(); ++j)
                {
                    x_grad[i].add_feat(mat[j], y_gard[i].data()[j]);
                }
            }
        }
