...
namespace ant
{
    ...
    struct HierDFS
    {
        ...
        static size_t repair_isolate_node(Graph<indexType> &G, rangeType &enter_node_ids, size_t node_num, size_t batch_size = 1000, HIdMapping *hidMapping = nullptr)
        {
            ...
            while (!next_visited.empty())
            {
                ...
                for (size_t i = 0; i < cur_visited_size; ++i)
                {
                    ...
                    if (hidMapping != nullptr)
                    {
                        auto eid = hidMapping->get_id_in_pre_level(cur_node_id);
                        if (eid >= 0 && flag[eid] == 0)
                        {
                            flag[eid] = 1;
                            next_visited.push(eid);
            ...
            for (size_t i = 0; i < node_num; ++i)
            {
                ...
                if (flag[i] == 0)
                {
                    if (hidMapping != nullptr && hidMapping->get_level(i) > 0)
                        continue;
                    cnt++;
                    auto edges = G[i];
                    for (size_t j = 0; j < edges.size(); ++j)
        ...
        template <typename indexType>
        static size_t repair_isolate_node(Graph<indexType> &G, size_t sp, size_t batch_size = 1000, HIdMapping *hidMapping = nullptr)
        {
            std::vector<size_t> enter_node_ids = {sp};
            return repair_isolate_node(G, enter_node_ids, hidMapping == nullptr ? G.size() : hidMapping->cur_node_num, batch_size, hidMapping);
        }
    };
