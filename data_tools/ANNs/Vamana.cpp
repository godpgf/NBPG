...
Graph_ build(BuildArgs build_args, QueryArgs query_args, bool repair_isolate = true)
{
    ...
    else
    {
        ...
        /*{
            // 统计入度
            std::vector<indexType> in_deg(G.size(), 0);
            ...
            // 获取节点索引并按入度降序排序
            std::vector<size_t> nodes(G.size());
            std::iota(nodes.begin(), nodes.end(), 0);
            std::sort(nodes.begin(), nodes.end(), [&](size_t a, size_t b)
                      { return in_deg[a] > in_deg[b]; });

            // ---------- 前10%高入度节点覆盖边比例 ----------
            size_t top_k = std::max(1, (int)std::ceil(0.1 * G.size()));
            indexType top_sum = 0;
            for (size_t i = 0; i < top_k; ++i)
            {
                top_sum += in_deg[nodes[i]];
            }
            double top_percentage = 100.0 * top_sum / total_edges;

            std::cout << "前 " << top_k << " 个高入度节点（占比 "
                      << 100.0 * top_k / G.size() << "%）覆盖了 "
                      << top_percentage << "% 的边。" << std::endl;

            // ---------- 后10%低入度节点平均入度 ----------
            size_t bottom_k = std::max(1, (int)std::floor(0.1 * G.size())); // 最后30%节点数
            indexType bottom_sum = 0;
            for (size_t i = G.size() - bottom_k; i < G.size(); ++i)
            {
                bottom_sum += in_deg[nodes[i]];
            }
            double bottom_avg = static_cast<double>(bottom_sum) / bottom_k;

            std::cout << "最后 " << bottom_k << " 个低入度节点（占比 "
                      << 100.0 * bottom_k / G.size() << "%）的平均入度为 "
                      << bottom_avg << std::endl;
        }*/
